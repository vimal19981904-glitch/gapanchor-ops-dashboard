...
export default function CommsCard({ commsData, onRefresh, loading, onOpenExcelModal }: Props) {
  ...
  return (
    <div className="glass-card animate-slide-up flex flex-col justify-between p-3 sm:p-5">
      <div>
        {/* Header */}
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 sm:gap-4 mb-4 pb-3 sm:pb-4 border-b border-border/50">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2 flex-wrap">
                <h2 className="text-base sm:text-lg font-bold" style={{ color: 'var(--text-primary)' }}>Leads Command Center</h2>
                <span className="px-2 py-0.5 text-[9px] sm:text-[10px] font-bold rounded-full bg-cyan-500/10 text-cyan-400 border border-cyan-500/20 shrink-0">
                  {total} Enquiries
                </span>
              </div>
          ...
          <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap shrink-0">
            <Link
              href="/enquiries"
              className="flex items-center gap-1.5 px-3 py-1.5 rounded-xl text-[10px] sm:text-xs font-bold bg-gradient-to-r from-cyan-600 to-indigo-600 hover:from-cyan-500 hover:to-indigo-500 text-white shadow-md shadow-cyan-500/20 transition-all cursor-pointer no-underline shrink-0 flex-1 sm:flex-auto justify-center"
            >
              <Filter size={13} />
              <span>Full Intelligence Center ({total})</span>
            </Link>
            <div className="flex items-center justify-center gap-1.5 rounded-xl sm:rounded-full px-2.5 py-1.5 sm:py-1 text-[10px] font-semibold border border-emerald-500/30 bg-emerald-500/10 text-emerald-400 shrink-0">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-2 sm:gap-3 mb-4">
          {[
            ...
            <div key={i} className={`rounded-xl border p-2.5 sm:p-3 ${m.alert ? 'border-rose-500/30 bg-rose-500/8' : 'border-border'}`} style={!m.alert ? { background: 'var(--surface-2)' } : {}}>
              ...
              <div className={`text-lg sm:text-xl font-extrabold mt-0.5 flex items-center gap-1.5 ${m.color}`}>
                {m.alert && <AlertTriangle size={14} className="sm:hidden" />}
                    ...
                    <div className="flex items-center gap-2.5 min-w-0 flex-1">
                      ...
                      <div className="min-w-0 flex-1">
                        <div className="flex items-center gap-2 flex-wrap">
                          ...
                          {enq.assignedToName && (
                            <span className="px-1.5 py-0.2 text-[9px] font-bold rounded-full bg-brand-500/15 text-brand-400 border border-brand-500/30 flex items-center gap-0.5 shrink-0">
                    ...
                    <div className="flex items-center gap-1.5 shrink-0">
                      ...
                      {currentUser?.role === 'admin' && (
                        <button
                          ...
                          className="p-1 rounded-lg border border-border text-tertiary hover:text-brand-400 hover:border-brand-500/40 hover:bg-brand-500/10 transition-all shrink-0"
                        >
                          <UserCheck size={13} />
                        </button>
                      )}
                      ...
                      {!isProcessed && (
                        <button
                          ...
                          className="btn-process !py-1 !px-2.5 !text-[11px] shrink-0"
                        >
                          <Zap size={11} />
